...
impl DwgParser {
    ...
    pub fn parse(&mut self, data: &[u8]) -> Result<DwgFile, IfcxError> {
        ...
        let ver = DwgVersion::from_code(&dwg.version_code).ok_or_else(|| {
            IfcxError::InvalidBinary(format!(
    ...
    fn parse_r13_r14(&mut self, data: &[u8], dwg: &mut DwgFile) -> Result<(), IfcxError> {
    ...
    fn parse_r2004(&mut self, data: &[u8], dwg: &mut DwgFile) -> Result<(), IfcxError> {
        if data.len() < 0x100 {
            return Err(IfcxError::InvalidBinary("R2004 file too short".into()));
        }
        dwg.codepage = u16::from_le_bytes([data[19], data[20]]);
        ...
        if section_page_size < 0x400 || section_page_size > 0x100000 {
            return Err(IfcxError::InvalidBinary("Invalid R2004 page size".into()));
        }
        if section_page_map_addr + 20 > data.len() || section_page_map_addr < 0x100 {
            return Err(IfcxError::InvalidBinary("Invalid R2004 page map address".into()));
        }
    ...
    fn decrypt_r2004_file_header(data: &[u8]) -> Result<Vec<u8>, IfcxError> {
        const ENC_SIZE: usize = 0x6C;
        if data.len() < 0x80 + ENC_SIZE {
            return Err(IfcxError::InvalidBinary("R2004: file too short for encrypted header".into()));
        }
    ...
    fn read_r2004_page_map(
        ...
        map_addr: usize,
        page_size: usize,
    ) -> Result<HashMap<i32, usize>, IfcxError> {
        if map_addr + 20 > data.len() {
            return Err(IfcxError::InvalidBinary("R2004: page map address out of bounds".into()));
        }
        ...
        if page_body + comp_size > data.len() {
            return Err(IfcxError::InvalidBinary("R2004: page map data out of bounds".into()));
        }
    ...
    /// between DWG versions and AutoCAD builds.
    ...
    fn parse_r2007_plus(&mut self, data: &[u8], dwg: &mut DwgFile) -> Result<(), IfcxError> {
        if data.len() < 0x100 {
            return Err(IfcxError::InvalidBinary("R2007 file too short".into()));
        }
        dwg.codepage = u16::from_le_bytes([data[19], data[20]]);
    ...
    fn try_r2007_page_pipeline(
        ...
        let enc_hdr = dwg_r2007::decrypt_file_header(data)?;

        // Read page map
        let (page_map, page_size) = dwg_r2007::read_page_map(data, &enc_hdr)?;
        if page_map.is_empty() {
            return Err(IfcxError::InvalidBinary("R2007: empty page map".into()));
        }
        ...
        let section_map_data = dwg_r2007::assemble_section(
            data, &page_map, page_size, section_map_id,
        )?;

        let sections = dwg_r2007::parse_section_map(&section_map_data);

        // Find sections by type or name
        let hdr_id = dwg_r2007::find_section(&sections, dwg_r2007::SECTION_HEADER, "Header");
        let cls_id = dwg_r2007::find_section(&sections, dwg_r2007::SECTION_CLASSES, "Classes");
        let hdl_id = dwg_r2007::find_section(&sections, dwg_r2007::SECTION_HANDLES, "Handles");
        let obj_id = dwg_r2007::find_section(&sections, dwg_r2007::SECTION_OBJECTS, "AcDbObjects");

        // Parse header
        if let Some(id) = hdr_id {
            let hdr_data = dwg_r2007::assemble_section(data, &page_map, page_size, id)?;
            dwg.header_vars = self.parse_header_vars_r2004(&hdr_data);
        }
        ...
        if let Some(id) = cls_id {
            let cls_data = dwg_r2007::assemble_section(data, &page_map, page_size, id)?;
            dwg.classes = self.parse_classes_r2004_section(&cls_data);
            for cls in &dwg.classes {
                self.class_map.insert(cls.class_number, cls.clone());
            }
        ...
        if let Some(id) = hdl_id {
            let hdl_data = dwg_r2007::assemble_section(data, &page_map, page_size, id)?;
            dwg.object_map = self.parse_object_map_r2004(&hdl_data);
        }

        // Assemble objects section
        let objects_data = if let Some(id) = obj_id {
            let d = dwg_r2007::assemble_section(data, &page_map, page_size, id)?;
        ...
        if dwg.objects.is_empty() && dwg.header_vars.len() <= 1 {
            return Err(IfcxError::InvalidBinary("R2007: page pipeline produced no results".into()));
        }
    ...
    fn parse_r2010_plus(&mut self, data: &[u8], dwg: &mut DwgFile) -> Result<(), IfcxError> {
        if data.len() < 0x100 {
            return Err(IfcxError::InvalidBinary("R2010+ file too short".into()));
        }
        dwg.codepage = u16::from_le_bytes([data[19], data[20]]);
    ...
    fn try_r2010_page_pipeline(
        ...
        Err(IfcxError::InvalidBinary("R2010+: no valid header config found".into()))
    }
        ...
        if !dwg.object_map.is_empty() {
            ...
            if let Some(pma) = pm_addr {
                ...
                if raw_pages.len() > 1000 {
                    ...
                    let stripped = super::dwg_r2007::strip_rs_parity(&raw_pages);
                    if stripped.len() > 500 {
    ...
    fn parse_single_object_r2000(
        ...
        if bit_start >= data.len() {
            return Err(IfcxError::InvalidBinary("Object start past end of data".into()));
        }
        if obj_size <= 0 {
...
pub fn decompress_r2004(src: &[u8], decompressed_size: usize) -> Result<Vec<u8>, IfcxError> {
