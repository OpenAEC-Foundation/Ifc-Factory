...
impl<'a> DwgBitReader<'a> {
    ...
    /// Read a single bit (B).
    pub fn read_bit(&mut self) -> Result<u8, IfcxError> {
        let byte_idx = self.bit_position >> 3;
        let bit_idx = 7 - (self.bit_position & 7);
        if byte_idx >= self.data.len() {
            return Err(IfcxError::InvalidBinary("DwgBitReader: read past end of data".into()));
        }
    ...
    pub fn read_bits(&mut self, count: usize) -> Result<u32, IfcxError> {
        let mut result = 0u32;
        for _ in 0..count {
    ...
    pub fn read_byte(&mut self) -> Result<u8, IfcxError> {
        Ok(self.read_bits(8)? as u8)
    }

    /// Read a signed 16-bit little-endian short (RS).
    pub fn read_short(&mut self) -> Result<i16, IfcxError> {
    ...
    pub fn read_raw_short(&mut self) -> Result<u16, IfcxError> {
        let lo = self.read_bits(8)? as u16;
        let hi = self.read_bits(8)? as u16;
        Ok(lo | (hi << 8))
    }
    ...
    pub fn read_long(&mut self) -> Result<i32, IfcxError> {
    ...
    pub fn read_raw_long(&mut self) -> Result<u32, IfcxError> {
    ...
    pub fn read_double(&mut self) -> Result<f64, IfcxError> {
        let mut bytes = [0u8; 8];
    ...
    pub fn read_bb(&mut self) -> Result<u8, IfcxError> {
        Ok(self.read_bits(2)? as u8)
    }

    /// Bit Short (BS) -- 2-bit prefix + variable payload.
    pub fn read_bs(&mut self) -> Result<i16, IfcxError> {
    ...
    pub fn read_bl(&mut self) -> Result<i32, IfcxError> {
        let prefix = self.read_bits(2)?;
        match prefix {
            0 => self.read_long(),
    ...
    pub fn read_bd(&mut self) -> Result<f64, IfcxError> {
        let prefix = self.read_bits(2)?;
        match prefix {
            0 => self.read_double(),
            1 => Ok(1.0),
    ...
    pub fn read_dd(&mut self, default: f64) -> Result<f64, IfcxError> {
        let prefix = self.read_bits(2)?;
        match prefix {
            0 => Ok(default),
            3 => self.read_double(),
    ...
    pub fn read_bt(&mut self) -> Result<f64, IfcxError> {
        if self.read_bit()? == 1 {
            Ok(0.0)
        } else {
            self.read_bd()
    ...
    pub fn read_be(&mut self) -> Result<(f64, f64, f64), IfcxError> {
        if self.read_bit()? == 1 {
            Ok((0.0, 0.0, 1.0))
        } else {
            let x = self.read_bd()?;
    ...
    pub fn read_h(&mut self) -> Result<(u8, u32), IfcxError> {
    ...
    pub fn read_t(&mut self, is_unicode: bool) -> Result<String, IfcxError> {
    ...
    pub fn read_cmc(&mut self) -> Result<i16, IfcxError> {
        self.read_bs()
    }
    ...
    pub fn read_modular_char(data: &[u8], pos: usize) -> Result<(i32, usize), IfcxError> {
        let mut result = 0i32;
        let mut shift = 0u32;
        ...
        loop {
            if p >= data.len() {
                return Err(IfcxError::InvalidBinary("modular_char: unexpected end".into()));
            }
            let b = data[p];
            p += 1;
            let cont = b & 0x80;
    ...
    /// Returns `(value, new_pos)`.
    pub fn read_modular_short(data: &[u8], pos: usize) -> Result<(i32, usize), IfcxError> {
        let mut result = 0u32;
        let mut shift = 0u32;
        let mut p = pos;
        ...
        loop {
            if p + 1 >= data.len() {
                return Err(IfcxError::InvalidBinary("modular_short: unexpected end".into()));
            }
            let lo = data[p];
            let hi = data[p + 1];
            p += 2;
    ...
    pub fn read_raw_bytes(&mut self, count: usize) -> Result<Vec<u8>, IfcxError> {
